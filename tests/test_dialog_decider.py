...
class TestDialogDecider(unittest.TestCase):

    def test_schema_lists_decisions_and_states(self) -> None:
        self.assertIn("approve", ACTION_DSL_SCHEMA["decision"])
        self.assertIn("none", ACTION_DSL_SCHEMA["decision"])
        self.assertIn("agy-allow-once", ACTION_DSL_SCHEMA["state"])
        self.assertIn("no-dialog", ACTION_DSL_SCHEMA["state"])

    def test_parse_valid_approval(self) -> None:
        action = parse_action_dsl(
            {"state": "agy-yes-no-y", "decision": "approve", "action": "type", "value": "y", "reason": "(y/n)"}
        )
        self.assertTrue(action.is_approval)
        self.assertEqual(action.state, "agy-yes-no-y")

    def test_parse_none_decision_must_be_empty(self) -> None:
        ok = parse_action_dsl({"state": "no-dialog", "decision": "none", "action": "", "value": "", "reason": ""})
        self.assertFalse(ok.is_approval)
        bad = parse_action_dsl({"state": "no-dialog", "decision": "none", "action": "key", "value": "Return"})
        self.assertFalse(bad.is_approval)
        self.assertIn("invalid-dsl", bad.reason)

    def test_parse_rejects_unknown_enums(self) -> None:
        for payload in (
            ...
            self.assertFalse(action.is_approval)
            self.assertIn("invalid-dsl", action.reason)

    def test_parse_rejects_unsafe_values(self) -> None:
        too_long = parse_action_dsl(
            {"state": "agy-yes-no-y", "decision": "approve", "action": "type", "value": "y" * 200}
        )
        self.assertFalse(too_long.is_approval)
        empty = parse_action_dsl(
            {"state": "agy-yes-no-y", "decision": "approve", "action": "type", "value": ""}
        )
        self.assertFalse(empty.is_approval)
        bad_click = parse_action_dsl(
            {"state": "select-option-1", "decision": "approve", "action": "click", "value": "12"}
        )
        self.assertFalse(bad_click.is_approval)
        good_click = parse_action_dsl(
            {"state": "select-option-1", "decision": "approve", "action": "click", "value": "640,480"}
        )
        self.assertTrue(good_click.is_approval)

    def test_parse_non_object_is_rejected(self) -> None:
        self.assertFalse(parse_action_dsl("approve").is_approval)
        self.assertFalse(parse_action_dsl([1, 2, 3]).is_approval)
        self.assertFalse(parse_action_dsl(None).is_approval)
    ...
    def test_extract_json_tolerates_fences_and_prose(self) -> None:
        ...
        self.assertEqual(payload["state"], "npm-proceed")
        self.assertIsNone(extract_json_object("no json here at all"))

    def test_decide_approves_yn_prompt(self) -> None:
        raw = json.dumps(
            {"state": "agy-yes-no-y", "decision": "approve", "action": "type", "value": "y", "reason": "(y/n)"}
        )
        ...
        self.assertTrue(action.is_approval)
        self.assertEqual(action.value, "y")

    def test_decide_fail_closed_on_transport_error(self) -> None:
        decider = fixed_decider(RuntimeError("connection refused"))
        action = decider.decide("Anything on screen")
        self.assertFalse(action.is_approval)
        self.assertIn("decider-unavailable", action.reason)

    def test_decide_fail_closed_on_garbage(self) -> None:
        decider = fixed_decider("I would press y maybe?")
        action = decider.decide("Proceed? (y/n)")
        self.assertFalse(action.is_approval)
        self.assertIn("invalid-dsl", action.reason)

    def test_decide_respects_model_refusal(self) -> None:
        raw = json.dumps({"state": "other-dialog", "decision": "none", "action": "", "value": "", "reason": "destructive warning"})
        decider = fixed_decider(raw)
        self.assertFalse(decider.decide("rm -rf / -- are you sure?").is_approval)

    def test_session_uses_decider_and_cooldown(self) -> None:
        raw = json.dumps(
        ...
        self.assertEqual(triggered, "agy-allow-once")
        self.assertEqual(kvm.calls, [("key", "Return")])

        # Same state inside the cooldown window must not fire again.
        kvm.calls.clear()
        self.assertIsNone(session.run_once())
        self.assertEqual(kvm.calls, [])

    def test_session_decider_none_decision_does_not_act(self) -> None:
    ...
    def test_session_decider_click_coordinates(self) -> None:
        raw = json.dumps(
            {"state": "select-option-1", "decision": "approve", "action": "click", "value": "300, 420", "reason": "option 1"}
        )
        kvm = FakeKVM("Select an option:\n 1) Allow")
        session = AutopilotSession(kvm=kvm, decider=fixed_decider(raw), verbose=False)
        self.assertEqual(session.run_once(), "select-option-1")
        self.assertEqual(kvm.calls, [("click", "300,420")])

    def test_dialog_action_defaults_are_inert(self) -> None:
        self.assertFalse(DialogAction().is_approval)
